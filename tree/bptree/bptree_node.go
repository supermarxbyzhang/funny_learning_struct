...
func (self *BpNode) Height() int {
	if !self.Internal() {
		...
		panic(errorss.BpTreeError("Internal node has no pointers but asked for height"))
	}
...
func (self *BpNode) internal_get_start(key types.Hashable) (i int, leaf *BpNode) {
	if !self.Internal() {
		panic(errorss.BpTreeError("Expected a internal node"))
	}
	i, has := self.find(key)
	if !has && i > 0 {
...
func (self *BpNode) internal_insert(key types.Hashable, value interface{}) (a, b *BpNode, err error) {
	if !self.Internal() {
		return nil, nil, errorss.BpTreeError("Expected a internal node")
	}
	i, has := self.find(key)
	if !has && i > 0 {
		// if it doesn't have it and the index > 0 then we have the next block
...
func (self *BpNode) internal_split(key types.Hashable, ptr *BpNode) (a, b *BpNode, err error) {
	...
	if self.has(key) {
		return nil, nil, errorss.BpTreeError("Tried to split an internal block on duplicate key")
	}
	a = self
...
func (self *BpNode) leaf_insert(key types.Hashable, value interface{}) (a, b *BpNode, err error) {
	if self.Internal() {
		return nil, nil, errorss.BpTreeError("Expected a leaf node")
	}
	if self.no_dup {
		i, has := self.find(key)
		if has {
...
func (self *BpNode) pure_leaf_split(key types.Hashable, value interface{}) (a, b *BpNode, err error) {
	if self.Internal() || !self.Pure() {
		return nil, nil, errorss.BpTreeError("Expected a pure leaf node")
	}
...
func (self *BpNode) put_kp(key types.Hashable, ptr *BpNode) error {
	if self.Full() {
		return errorss.BpTreeError("Block is full.")
	}
	if !self.Internal() {
		return errorss.BpTreeError("Expected a internal node")
	}
	i, has := self.find(key)
	if has {
		return errorss.BpTreeError("Tried to insert a duplicate key into an internal node")
	} else if i < 0 {
		panic(errorss.BpTreeError("find returned a negative int"))
	} else if i >= cap(self.keys) {
		panic(errorss.BpTreeError("find returned a int > than cap(keys)"))
	}
...
func (self *BpNode) put_kv(key types.Hashable, value interface{}) error {
	...
	if self.Internal() {
		return errorss.BpTreeError("Expected a leaf node")
	}
	i, _ := self.find(key)
	if i < 0 {
		panic(errorss.BpTreeError("find returned a negative int"))
...
func (self *BpNode) leaf_remove(key, stop types.Hashable, where types.WhereFunc) (a *BpNode, err error) {
	if self.Internal() {
		return nil, errorss.BpTreeError("Expected a leaf node")
	}
	a = self
...
func (self *BpNode) remove_key_at(i int) error {
	if i >= len(self.keys) || i < 0 {
		return errorss.BpTreeError("i, %v, is out of bounds, %v, %v %v.", i, len(self.keys), len(self.values), self)
	}
...
func (self *BpNode) remove_value_at(i int) error {
	if i >= len(self.values) || i < 0 {
		return errorss.BpTreeError("i, %v, is out of bounds, %v.", i, len(self.values))
	}
...
func (self *BpNode) remove_ptr_at(i int) error {
	if i >= len(self.pointers) || i < 0 {
		return errorss.BpTreeError("i, %v, is out of bounds, %v.", i, len(self.pointers))
	}
...
func insert_linked_list_node(n, prev, next *BpNode) {
	if (prev != nil && prev.next != next) || (next != nil && next.prev != prev) {
		panic(errorss.BpTreeError("prev and next not hooked up"))
	}
	n.prev = prev
	n.next = next
	if prev != nil {
...
func balance_nodes(a, b *BpNode) {
	if len(b.keys) != 0 {
		panic(errorss.BpTreeError("b was not empty"))
	}
	if !a.Full() {
		panic(errorss.BpTreeError("a was not full", a))
	}
	if cap(a.keys) != cap(b.keys) {
		panic(errorss.BpTreeError("cap(a.keys) != cap(b.keys)"))
	}
	if cap(a.values) != cap(b.values) {
		panic(errorss.BpTreeError("cap(a.values) != cap(b.values)"))
	}
	if cap(a.pointers) != cap(b.pointers) {
		panic(errorss.BpTreeError("cap(a.pointers) != cap(b.pointers)"))
	}
	m := len(a.keys) / 2
	...
	if cap(a.values) > 0 {
		if cap(a.values) != cap(a.keys) {
			panic(errorss.BpTreeError("cap(a.values) != cap(a.keys)"))
		}
		b.values = b.values[:lim]
	}
	if cap(a.pointers) > 0 {
		if cap(a.pointers) != cap(a.keys) {
			panic(errorss.BpTreeError("cap(a.pointers) != cap(a.keys)"))
		}
