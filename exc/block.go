...
func (b *Block) run() Throwable {
	...
	if err != nil {
		...
		for _, c := range b.catches {
			// errorss.Logf("DEBUG", "trying to catch %v with %v %v %v", t, c.exception, t.ConvertibleTo(c.exception), t.AssignableTo(c.exception))
...
func isa(a, b reflect.Type) bool {
	// errorss.Logf("DEBUG", "isa a %v b %v a.Kind() %v", a, b, a.Kind())
	if is(a, b) {
		return true
	}
...
func is(a, b reflect.Type) bool {
	// errorss.Logf("DEBUG", "is a %v b %v %v %v", a, b, a.ConvertibleTo(b), a.AssignableTo(b))
	if a.AssignableTo(b) {
		return true
	}
	return false
