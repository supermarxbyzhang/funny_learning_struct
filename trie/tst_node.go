...
func (n *TSTNode) insert(key ByteSlice, val interface{}, d int) (*TSTNode, error) {
	if d >= len(key) {
		return nil, errorss.TSTError("depth exceeds key length")
	}
	if key[len(key)-1] != END {
		return nil, errorss.TSTError("key must end in 0")
	}
	if n == nil {
...
func (b *TSTNode) split(a *TSTNode, d int) (t *TSTNode, err error) {
	if !a.accepting {
		return nil, errorss.TSTError("`a` must be an accepting node")
	} else if !b.accepting {
		return nil, errorss.TSTError("`b` must be an accepting node")
	}
	if d >= len(b.key) {
		return nil,
			errorss.TSTError("depth of split exceeds key length of b")
	}
	t = NewTSTNode(b.ch)
	b = b.Copy()
	a = a.Copy()
