...
func (self *TST) ValidateKey(key []byte) error {
	if key == nil {
		return errorss.InvalidKey(key, "key is nil")
	}
	if len(key) == 0 {
		return errorss.InvalidKey(key, "len(key) == 0")
	}
	for _, ch := range key {
		if ch == 0 {
			return errorss.InvalidKey(key, "key contains a null byte")
		}
	}
	return nil
}
...
func (self *TST) Get(key []byte) (value interface{}, err error) {
	...
	for next != nil {
		if next.n == nil {
			return nil, errorss.NotFound(key)
