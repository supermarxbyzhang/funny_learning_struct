...
func (l *List) Get(i int) (item types.Hashable, err error) {
	if i < 0 || i >= len(l.list) {
		return nil, errorss.Errorf("Access out of bounds. len(*List) = %v, idx = %v", len(l.list), i)
	}
	return l.list[i], nil
}
...
func (l *List) Set(i int, item types.Hashable) (err error) {
	if i < 0 || i >= len(l.list) {
		return errorss.Errorf("Access out of bounds. len(*List) = %v, idx = %v", len(l.list), i)
	}
	l.list[i] = item
	return nil
}
...
func (l *List) expand() error {
	if l.fixed {
		return errorss.Errorf("Fixed size list is full!")
	}
	list := l.list
